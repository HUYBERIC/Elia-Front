...
const Calendar = () => {
  ...
  // Fetch events
  useEffect(() => {
    fetch("https://elia-back-et5at9thc-huyberics-projects.vercel.app/api/duties")
      .then((res) => res.json())
      .then((data) => {
        setEvents(
          data.map((event) => ({
  ...
  const handleDateClick = async (info) => {
    ...
    try {
      const response = await fetch("https://elia-back-et5at9thc-huyberics-projects.vercel.app/api/duties", {
        method: "POST",
  ...
  const handleEventClick = async (clickInfo) => {
    if (window.confirm(`Supprimer "${clickInfo.event.title}" ?`)) {
      await fetch(`https://elia-back-et5at9thc-huyberics-projects.vercel.app/api/duties/${clickInfo.event.id}`, { method: "DELETE" });
