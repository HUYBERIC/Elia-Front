...
const Notification = () => {
  ...
  const handleDecline = async (requestId) => {
    try {
      const response = await fetch(`http://localhost:5000/api/requests/${requestId}/decline`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
          ...
          <div key={request._id} className="notification-content">
            ...
            <div className="notification-text">
              <div className="notification-message">
                ...
                <p className="time">{timeSince(new Date(request.createdAt))} ago</p>
                <p>Emergency Level: <strong>{request.emergencyLevel}</strong></p>
              </div>
              <div className="notification-actions">
                <button className="btn primary" onClick={() => handleAccept(request._id)}>Accept</button>
                <button className="btn outline" onClick={() => handleDecline(request._id)}>Decline</button>
              </div>
            </div>
          </div>
        ))
